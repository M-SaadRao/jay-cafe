      ...
      <header className="bg-white sticky top-0 z-[1000] border-t-4 p-3 sm:p-0 border-secondary">
        <div className="container mx-auto flex justify-between items-center md:px-2">
          <div className="flex gap-6 items-center">
            ...
            >
              <FaTruck className="text-lg playFont" />
              <span>Try Our Online Ordering</span>
            </Button>
          </div>

          <div className="hidden lg:flex justify-center items-center playFont">
            <div className="mr-3 flex gap-4 items-center">
              {menuItems.map((item) => (
                <NavLink
                  ...
                  to={`/${item.key}`}
                  className={({ isActive }) =>
                    `text-[17px] p-3 py-7 leading-none ${
                      isActive ? "text-white bg-secondary shadow-md" : ""
                    }`
                  }
                >
                  {item.label}
              ...
              <NavLink
                to="/contact"
                className={({ isActive }) =>
                  `text-[17px] p-3 py-7 playFont leading-none ${
                    isActive ? "text-white bg-secondary shadow-md" : ""
                  }`
                }
              >
          ...
          <div className="mt-4 space-y-3 px-2">
            {menuItems.map((item) => (
              <NavLink
                ...
                to={`/${item.key}`}
                className={({ isActive }) =>
                  `block p-3 rounded-lg text-[14px] font-medium ${
        ...
        {cartItems.length > 0 && (
          <div className="lg:hidden flex justify-between items-center gap-3 px-5 py-2 bg-primary w-full text-white text-center fixed bottom-0 left-0 mt-4">
            <div>
              <p className="text-[10px] m-0 p-0 leading-none">
                item {cartItems.length}
              </p>
              <p className="text-[13px] font-medium mt-1 p-0 leading-none">
                ${cartTotalPrice}
              </p>
            </div>
            <div
